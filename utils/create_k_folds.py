...
parser.add_argument('--input', '-i', type=str, help='Training file to split into chunks.')
...
def conllu_reader(infile):
    ...
    file = codecs.open(infile, 'r', encoding=args.encoding)
...
def generate_k_folds(in_sentences, k):
    """Divides input sentences into k folds.
    
    Args:
        in_sentences: list of lists containing conllu sentences.
    ...
    dataset_copy = list(in_sentences)  
    sents_per_fold = int(len(in_sentences) / k)
    remainder = int(len(in_sentences) % k)
...
# create input sentences and token/sentence counts
in_sentences, token_counts, sent_count = conllu_reader(args.input)

# turn input sentences into k folds
folded_dataset = generate_k_folds(in_sentences, args.k)
...
for i, fold in enumerate(folded_dataset):
    ...
    conllu_train = tbid + '-ud-train.conllu' + '.' + 'split-'+ str(i)
