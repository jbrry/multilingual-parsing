  ...
  if [ ${model_type} == 'monolingual' ]
    then echo "training monolingual model(s)..."
     ...
     for tbid in da_ddt sv_talbanken no_nynorsk no_bokmaal; do
       for filepath in ${GOLD_DIR}/*/$tbid-ud-train.conllu; do 
        ...
        if [ ${data_type} == 'gold' ]; then
          ...
          # output of ./scripts/predict_with_cross_val.sh
          export TRAIN_DATA_PATH=${TB_DIR}/${tb_name}/${tbid}-ud-train-${RANDOM_SEED}.conllu
          export DEV_DATA_PATH=${TB_DIR}/${tb_name}/${tbid}-ud-dev-${RANDOM_SEED}.conllu
        fi
    ...
    if [ ${data_type} == 'gold' ]; then
      ...
      export TRAIN_DATA_PATH=${TB_DIR}/**/*-ud-train-${RANDOM_SEED}.conllu
      export DEV_DATA_PATH=${TB_DIR}/**/*-ud-dev-${RANDOM_SEED}.conllu
    fi
