...
@TokenEmbedder.register("embedding_multilang")
class EmbeddingMultilang(TokenEmbedder):
    """
    ...
    pretrained_files : ``Dict[str, str]``, (optional, default=None)
        A dictionary of language identifier to pretrained embeddings file.
    ...
    def __init__(self,
                 ...
                 pretrained_files: Dict[str, str] = None) -> None:
        super(EmbeddingMultilang, self).__init__()
...
class EmbeddingsTextFile(Iterator[str]):
    ...
    def __init__(self,
        ...
        if zipfile.is_zipfile(main_file_local_path):  # ZIP archive
            ...
            # when necessary.
            package = None
            if extension in ['.txt', '.vec', '.vectors']:
                package = io
            elif extension == '.gz':
                import gzip
                package = gzip
            ...
            self._handle = package.open(main_file_local_path, 'rt', encoding=encoding, errors='ignore')  # type: ignore
